...
def expected_sarsa_for_grid_world(env: SingleAgentEnv, lr: float, discount: float, epsilon: float, max_iter: int) \
    ...
    for _ in tqdm(range(max_iter)):
        ...
        while not env.is_game_over():
            ...
            if env.is_game_over():
                ...
                sum_total = sum([pi[s_p][action] * q[s_p][action] for action in next_actions]) - q[s][chosen_action]
                q[s][chosen_action] += lr * (r + discount * sum_total)
